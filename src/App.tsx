...
type AppView = 'menu' | 'game' | 'editor' | 'irregular-game' | 'achievements' | 'dailyChallenge' | 'multiplayer' | 'shop' | 'savedPuzzles';
...
const AppContent: React.FC = () => {
  ...
  // 新增：用于控制编辑器初始step
  const [editorInitialStep, setEditorInitialStep] = useState<'upload' | 'crop' | 'settings' | 'preview'>('upload');
  ...
  // 新增：跳转到本地存档页面
  const handleOpenSavedPuzzles = () => {
    setCurrentView('savedPuzzles');
  };

  const handleOpenEditor = (step: 'upload' | 'crop' | 'settings' | 'preview' = 'upload') => {
  ...
  const renderCurrentView = () => {
    switch (currentView) {
      case 'menu':
        return (
          <MainMenu
            ...
            onOpenSavedPuzzles={handleOpenSavedPuzzles} // 新增入口
          />
        );
      case 'savedPuzzles':
        return <SavedPuzzlesPage onOpenEditor={handleOpenEditor} />;
      ...
      case 'editor':
        return (
          <PuzzleEditor onBackToMenu={handleBackToMenu} onStartGame={handleStartGame} initialStep={editorInitialStep} />
        );
